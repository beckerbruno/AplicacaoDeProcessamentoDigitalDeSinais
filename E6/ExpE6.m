...
% Experi�ncia E6: Convers�o de taxas de amostragem
...
zi = zeros([1 NFILT]); % condi��es iniciais para o filtro da FuncaoE6
...
title('Audio In - Espectro de Frequ�ncias');
...
xlabel('Frequencia (Hz)'), ylabel('Magnitude (norm.)'),
title('Audio Out - Espectro de Frequ�ncias');
...
while(toc < TT),
    ...
    % processa dados
    [yh3 zi] = FuncaoE6(yh1, FA_AD, FA_DA, zi);
    yh3 = yh3(1:length(xh3));
...
release(ARec); % fecha dispositivo de entrada de �udio
clear ARec % remove ARec do workspace
release(APly); % fecha dispositivo de sa�da de �udio
